    ...
    return (
        ...
        >
            <nav className={`flex items-center gap-6 px-8 py-4 transition-all duration-500 ${isScrolled
                ? 'bg-white/80 backdrop-blur-2xl rounded-full shadow-[0_8px_32px_rgba(31,38,135,0.1)] ring-1 ring-slate-900/5 border border-slate-200/50'
                : 'bg-white/40 backdrop-blur-xl rounded-full shadow-sm ring-1 ring-slate-900/5 border border-slate-200/20'
                }`}>
                    ...
                    {navLinks.map((link) => (
                        ...
                        >
                            ...
                            {activePage === link.href && (
                                <span className="absolute -bottom-2 left-0 right-0 h-0.5 bg-teal-500 rounded-full shadow-[0_0_8px_rgba(20,184,166,0.4)]" />
                            )}
                        </Link>
                    ))}
                </div>
